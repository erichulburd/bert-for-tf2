...
flags.DEFINE_string("data_dir", 'data', "The data directory with input and output files.")

flags.DEFINE_string("output_subdir",
                    datetime.datetime.now().isoformat(),
                    "The output directory where the model checkpoints will be written or read.")

flags.DEFINE_string("bert_directory", 'cased_L-12_H-768_A-12',
                    "Direct with BERT config, checkpoint and vocabulary")

flags.DEFINE_string("layer_config", None, "The file containing hyperparameters for current run.")

flags.DEFINE_bool(
    "read_features_from_file", True,
    "Read train and eval features from tf_record and pickle files. \
        If false, write these files for future runs.")

flags.DEFINE_string("train_squad_file", 'train-v2.0.json',
                    "SQuAD json for training. E.g., train-v2.0.json")

flags.DEFINE_string("eval_squad_file", 'dev-v2.0.json',
                    "SQuAD json for predictions. E.g., dev-v2.0.json or test-v1.1.json")

flags.DEFINE_string("init_checkpoint", None,
                    "Initial checkpoint (usually from a pre-trained BERT model).")

flags.DEFINE_bool(
    "do_lower_case", False, "Whether to lower case the input text. Should be True for uncased "
...
def read_squad_examples(input_file, is_training):
    ...
    examples = []
    reached_n_max_examples = False
    for entry in input_data:
        if reached_n_max_examples:
            break
        for paragraph in entry["paragraphs"]:
            ...
            for qa in paragraph["qas"]:
                ...
                if FLAGS.n_max_examples is not None and len(examples) == FLAGS.n_max_examples:
                    reached_n_max_examples = True
...
def get_final_text(pred_text, orig_text, do_lower_case):
    ...
    if start_position == -1:
        if FLAGS.verbose_logging:
            tf.compat.v1.logging.info("Unable to find text: '%s' in '%s'" % (pred_text, orig_text))
        return orig_text
...
def _process_train_features_if_necessary():
    ...
    if FLAGS.read_features_from_file:
        ...
        with tf.io.gfile.GFile(data_filename, "w") as f:
            json.dump(metadata, f)

    tf.compat.v1.logging.info("***** Running training *****")
    tf.compat.v1.logging.info("  Num orig examples = %d", metadata['n_train_examples'])
    tf.compat.v1.logging.info("  Num split examples = %d", metadata['num_features'])
...
def _process_eval_features_if_necessary():
    ...
    if FLAGS.read_features_from_file:
        with tf.io.gfile.GFile(examples_file, "r") as f:
            eval_examples = pickle.load(f)

        with tf.io.gfile.GFile(features_file, "r") as f:
            eval_features = pickle.load(f)
    else:
        eval_examples = read_squad_examples(input_file=EVAL_SQUAD_FILE, is_training=False)
        with tf.io.gfile.GFile(examples_file, "wb") as f:
        ...
        with tf.io.gfile.GFile(features_file, "wb") as f:
