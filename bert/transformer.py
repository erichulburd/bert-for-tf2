...
class TransformerSelfAttentionLayer(Layer):
    ...
    def _construct(self, params: Params):
        if params.hidden_size % params.num_heads != 0:
            raise ValueError("The hidden_size:[{}] is not a multiple of num_heads:[{}]".format(
    ...
    def call(self, inputs, mask=None, training=None):
        ...
        attention_output = self.attention_projector([attention_head, layer_input],
                                                    mask=mask,
                                                    training=training)
...
class SingleTransformerEncoderLayer(Layer):
    ...
    def build(self, input_shape):
        ...
        self.self_attention_layer = TransformerSelfAttentionLayer.from_params(self.params,
                                                                              name="attention")
        self.intermediate_layer = keras.layers.Dense(name="intermediate",
                                                     units=self.params.intermediate_size,
                                                     activation=self.get_activation(
                                                         self.params.intermediate_activation),
                                                     kernel_initializer=self.create_initializer())
...
class TransformerEncoderLayer(Layer):
    ...
    def build(self, input_shape):
        ...
        if self.params.shared_layer:
            # ALBERT: share params
            self.shared_layer = SingleTransformerEncoderLayer.from_params(self.params,
                                                                          name="layer_shared")
        else:
            # BERT
