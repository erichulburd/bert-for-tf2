...
def embedding_lookup(input_ids,
  ...
  """
    ...
    embedding_table = tf.compat.v1.get_variable(name=word_embedding_name,
...
def attention_layer(from_tensor,
  ...
  """
    ...
    # `key_layer` = [B*T, N*H]
    key_layer = tf.compat.v1.layers.dense(to_tensor_2d,
                                          num_attention_heads * size_per_head,
                                          activation=key_act,
                                          name="key",
                                          kernel_initializer=create_initializer(initializer_range))
    ...
    # `key_layer` = [B, N, T, H]
    key_layer = transpose_for_scores(key_layer, batch_size, num_attention_heads, to_seq_length,
                                     size_per_head)
