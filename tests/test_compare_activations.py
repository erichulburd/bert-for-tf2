...
class CompareBertActivationsTest(AbstractBertTest):
    ...
    def load_stock_model(model_dir, max_seq_len):
        ...
        (assignment_map,
         initialized_var_names) = get_assignment_map_from_checkpoint(tvars, bert_ckpt_file)
    ...
    def predict_on_stock_model(model_dir, input_ids, input_mask, token_type_ids):
        ...
        (s_model, pl_input_ids, pl_token_type_ids,
         pl_mask) = CompareBertActivationsTest.load_stock_model(model_dir, max_seq_len)
    ...
    def test_finetune(self):
        ...
        pres = model.predict([input_ids,
                              token_type_ids])  # just for fetching the shape of the output
        print("pres:", pres.shape)

        model.fit(x=(input_ids, token_type_ids), y=np.zeros_like(pres), batch_size=2, epochs=2)
